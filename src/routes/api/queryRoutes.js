...
route.post('/', async (req, res, next) => {
    ...
    if (result._id) {
        res.status(200).json({ status: 200, message: "this will save an query", data: result })
    } else {
...
route.delete('/:id', async (req, res, next) => {
    ...
    if (result === true) {
        res.status(200).json({ status: 200, message: "this will delete one query", data: `deleted ${req.params.id}  successfully ` })
    } else {
        res.status(404).json({ status: 404, message: result, data: '' })
    }
})
